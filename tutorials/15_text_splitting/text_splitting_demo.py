...
'''
    ...
    def demo_character_splitter(self, text: str) -> List[Document]:
        ...
        for i, doc in enumerate(documents[:3]):  # 只显示前3个
            logger.info(f"片段 {i+1} (长度: {len(doc.page_content)}): {doc.page_content[:100]}...")
    ...
    def demo_markdown_splitter(self, markdown_text: str) -> List[Document]:
        ...
        for i, doc in enumerate(documents):
            ...
            logger.info(f"内容 (长度: {len(doc.page_content)}): {doc.page_content[:100]}...")
    ...
    def evaluate_splitting_quality(self, documents: List[Document], method_name: str) -> Dict[str, Any]:
        """评估切分质量"""
        if not documents:
    ...
    def save_results_to_file(self, methods_and_results: List, output_file: str = "splitting_results.json"):
        """保存结果到文件"""
        results = []
        for method_name, docs, metrics in methods_and_results:
            result = {
                "method": method_name,
                "metrics": metrics,
                "sample_chunks": [doc.page_content[:200] + "..." for doc in docs[:3]]  # 保存前3个片段的样本
            }
...
def main():
    ...
    if os.path.exists(pdf_path):
        ...
        计算机视觉是一门研究如何使机器"看"的科学，更进一步的说，就是是指用摄影机和电脑代替人眼对目标进行识别、跟踪和测量等机器视觉，并进一步做图形处理。""" * 5
