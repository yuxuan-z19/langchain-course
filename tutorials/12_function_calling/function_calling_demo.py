...
class TimeTool(BaseTool):
    ...
    class TimeInput(BaseModel):
        format: str = Field(description="时间格式：'current'获取当前时间，'date'获取日期，'time'获取时间")
...
class FileOperationTool(BaseTool):
    ...
    class FileInput(BaseModel):
        ...
        content: str = Field(default="", description="写入的内容（仅在write操作时需要）")
            ...
            try:
                ...
                if message.tool_calls:
                    ...
                    if function_name == "calculator":
                        tool_result = self._execute_calculator(function_args["expression"])
                    elif function_name == "get_current_time":
                        tool_result = self._execute_time_query(function_args.get("format", "datetime"))
                    else:
                        tool_result = "未知的工具函数"
                    ...
                    messages = [
                        ...
                        {"role": "assistant", "content": None, "tool_calls": [tool_call.dict()]},
                        {
                            "role": "tool",
                            "content": tool_result,
