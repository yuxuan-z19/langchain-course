...
# 初始化模型
model = ChatOpenAI(model="deepseek-chat", temperature=0.7, max_tokens=1000)
...
def demo_basic_llm_chain():
    ...
    # 创建 LLMChain
    chain = LLMChain(llm=model, prompt=prompt, verbose=True)  # 显示详细信息
...
def demo_output_parser_chain():
    ...
    # 创建链
    chain = LLMChain(llm=model, prompt=prompt, output_parser=parser, verbose=True)
...
def demo_simple_sequential_chain():
    ...
    sequential_chain = SimpleSequentialChain(
        chains=[outline_chain, story_chain], verbose=True
    )
...
def demo_sequential_chain():
    ...
    analysis_chain = LLMChain(
        llm=model, prompt=analysis_prompt, output_key="analysis"  # 指定输出键名
    )
    ...
    plan_chain = LLMChain(llm=model, prompt=plan_prompt, output_key="plan")
    ...
    cost_chain = LLMChain(llm=model, prompt=cost_prompt, output_key="cost")
    ...
    try:
        # 运行顺序链
        result = sequential_chain({"product_idea": "一个基于AI的个人健康管理应用"})
...
请提供准确和有用的信息。"""
    ...
    try:
        ...
        default_prompt = PromptTemplate(
            template=general_template, input_variables=["input"]
        )
        ...
        def simple_router(question: str) -> str:
            ...
            if any(word in question_lower for word in ["计算", "数学", "方程", "求解"]):
                ...
                word in question_lower for word in ["代码", "编程", "算法", "python"]
            ):
                return "programming"
            else:
                return "general"
...
def demo_performance_optimization():
    ...
    test_prompt = PromptTemplate(
        input_variables=["number"], template="请计算 {number} 的平方"
    )

    test_chain = LLMChain(llm=model, prompt=test_prompt)
