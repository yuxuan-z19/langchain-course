...
class ChromaDatabaseDemo:
    ...
    def _setup_embedding_function(self):
        ...
        try:
            ...
            if embedding_config and embedding_config.get('api_key'):
                logger.info(f"使用配置的向量模型: {embedding_config.get('model_name', 'default')}")
                
                # 使用OpenAI兼容的嵌入函数
                self.embedding_function = embedding_functions.OpenAIEmbeddingFunction(
                    api_key=embedding_config['api_key'],
                    api_base=embedding_config.get('base_url', 'https://api.openai.com/v1'),
    ...
    def load_civil_code_documents(self, pdf_path: str = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if pdf_path is None:
                ...
                pdf_path = os.path.join(project_root, "docs", "中华人民共和国民法典.pdf")
    ...
    def add_documents(self, documents: List[Dict[str, Any]]) -> bool:
        ...
        try:
            ...
            logger.info(f"成功添加所有 {len(documents)} 个文档，耗时: {insert_time:.2f}秒")
            return True
    ...
    def demo_basic_operations(self):
        ...
        try:
            ...
            if filtered_results['ids']:
                for doc_id, content in zip(filtered_results['ids'], filtered_results['documents']):
    ...
    def demo_similarity_search(self):
        ...
        for query in search_queries:
            try:
                ...
                for i, (doc_id, content, distance, metadata) in enumerate(zip(
    ...
    def demo_mmr_search(self):
        ...
        try:
            ...
            selected_indices = self._mmr_selection(
                candidates, 
                k=5, 
                lambda_param=0.7  # 0.7权重给相关性，0.3给多样性
            )
    ...
    def _mmr_selection(self, candidates: Dict, k: int = 5, lambda_param: float = 0.5) -> List[int]:
        """
        实现MMR选择算法
            ...
            for idx in remaining:
                ...
                if selected:
                    ...
                    for sel_idx in selected:
                        ...
                        content_diversity = abs(len(current_content) - len(selected_content)) / max(len(current_content), len(selected_content))
                        
                        current_category = candidates['metadatas'][0][idx].get('category', '')
                        selected_category = candidates['metadatas'][0][sel_idx].get('category', '')
                        category_diversity = 1.0 if current_category != selected_category else 0.0
                ...
                # MMR分数
                mmr_score = lambda_param * relevance + (1 - lambda_param) * min_diversity
        ...
        for case in search_cases:
            try:
                ...
                if results['ids'][0]:
                    ...
                    for i, (doc_id, content, distance, metadata) in enumerate(zip(
                        fallback_results['ids'][0], fallback_results['documents'][0],
                        fallback_results['distances'][0], fallback_results['metadatas'][0]
                    )):
                        similarity = 1 - distance
                        logger.info(f"  结果{i+1}: (相似度: {similarity:.3f})")
                        logger.info(f"    ID: {doc_id}")
    ...
    def demo_batch_operations(self):
        ...
        try:
            ...
            # 显示部分结果
            logger.info("\n部分批量查询结果:")
            for i, (query, result) in enumerate(zip(batch_queries[:2], batch_results[:2])):
                logger.info(f"  查询{i+1}: {query}")
                if result['ids'][0]:
                    best_match = result['documents'][0][0]
                    similarity = 1 - result['distances'][0][0]
                    logger.info(f"    最佳匹配 (相似度: {similarity:.3f}): {best_match[:100]}...")
