...
""".format(
        ...
        def human_assistance(
            ...
            state_update = Command(
                update={
                    ...
                    "messages": [
                        ToolMessage(
                            content=f"等待人工验证: {query}", tool_call_id=tool_call_id
                        )
                    ],
                }
            )
        ...
        def search_entity_info(
            ...
            try:
                    ...
                    for i, result in enumerate(results[:3], 1):
                        if isinstance(result, dict):
                            ...
                            content = result.get(
                                "content",
                                result.get(
                                    "snippet", result.get("description", "无内容")
                                ),
                            )
                            ...
                            search_results_list.append(
                                {"type": "text", "content": str(result)}
                            )
                else:
                    formatted_output += str(results)
                    search_results_list.append(
                        {"type": "text", "content": str(results)}
                    )
                ...
                state_update = {
                    ...
                    "messages": [
                        ToolMessage(content=formatted_output, tool_call_id=tool_call_id)
                    ],
                }
                ...
                error_msg = (
                    f"❌ 搜索时发生错误: {str(e)}。请检查 TAVILY_API_KEY 是否正确配置。"
                )
                return Command(
                    update={
                        "messages": [
    ...
    def chat_with_custom_state(
        self, message: str, thread_id: str = "default", stream: bool = True
    ...
    def resume_execution(
        ...
        try:
            ...
            for event in self.graph.stream(
                Command(resume={"data": user_input}), config
            ):
                for node_name, node_output in event.items():
                    if node_name == "agent" and "messages" in node_output:
                        latest_message = node_output["messages"][-1]
            ...
            try:
                ...
                if user_input:
                    ...
                    if final_state:
                        ...
                        print(f"生日/发布日期: {final_state.get('birthday', '未设置')}")
                        print(
                            f"验证状态: {final_state.get('verification_status', 'pending')}"
                        )
                    break
                else:
                    print("❌ 请输入有效的验证信息")
