...
try:
    ...
    from utils.config import load_environment, load_embedding_config, get_config_dict
...
class VectorDatabase:
    ...
    def __init__(self, collection_name: str = "multi_query_demo", persist_directory: str = "./demo_multi_query_db"):
...
class MultiQueryRAG:
    ...
    def _merge_results_with_rrf(self, all_results: List[Tuple[List[str], List[float]]], queries: List[str]) -> Tuple[List[str], List[float], Dict[str, Any]]:
        ...
        # 记录每个查询的结果
        for i, (docs, scores, query) in enumerate(zip(all_results, [r[1] for r in all_results], queries)):
            query_info = {
                "query": query,
                "documents": docs,
                "scores": scores,
        ...
        debug_info["final_ranking"] = [(doc[:100] + "..." if len(doc) > 100 else doc, score) for doc, score in sorted_items[:5]]
    ...
    def _merge_results(self, all_results: List[Tuple[List[str], List[float]]]) -> Tuple[List[str], List[float]]:
        """合并多个查询的检索结果（保持向后兼容）"""
        merged_docs, merged_scores, _ = self._merge_results_with_rrf(all_results, [])
        return merged_docs, merged_scores
    
    def query(self, question: str, top_k: int = 5, show_process: bool = False) -> MultiQueryResult:
        """执行多查询RAG
        ...
        try:
            ...
            if show_process or self.debug_mode:
                print(f"✅ 生成了 {len(queries)} 个查询 (耗时: {query_generation_time:.3f}秒)")
            ...
            # 3. 使用RRF合并结果
            merged_docs, merged_scores, debug_info = self._merge_results_with_rrf(all_results, queries)
...
请基于上下文信息提供准确、详细的回答。如果上下文中没有相关信息，请说明无法回答。"""
    ...
    def compare_strategies(self, query: str) -> ComparisonResult:
        ...
        if traditional_result.scores and multi_query_result.scores:
            traditional_avg = sum(traditional_result.scores) / len(traditional_result.scores)
            multi_query_avg = sum(multi_query_result.scores) / len(multi_query_result.scores)
            if traditional_avg > 0:
                improvement_score = (multi_query_avg - traditional_avg) / traditional_avg
...
class PerformanceEvaluator:
    ...
    def evaluate_strategies(self, test_queries: List[str]) -> Dict[str, Any]:
        ...
        multi_query_avg_time = sum(r.processing_time for r in multi_query_results) / len(multi_query_results)
        
        traditional_avg_docs = sum(len(r.documents) for r in traditional_results) / len(traditional_results)
        multi_query_avg_docs = sum(len(r.documents) for r in multi_query_results) / len(multi_query_results)
        
        traditional_avg_score = sum(sum(r.scores) / len(r.scores) if r.scores else 0 for r in traditional_results) / len(traditional_results)
        multi_query_avg_score = sum(sum(r.scores) / len(r.scores) if r.scores else 0 for r in multi_query_results) / len(multi_query_results)
        ...
        return {
            ...
            "improvement_ratio": multi_query_avg_score / traditional_avg_score if traditional_avg_score > 0 else 0
        }
    ...
    @staticmethod
    def print_query_result(result: MultiQueryResult, title: str, show_details: bool = False):
        """打印查询结果
        ...
        if result.documents:
            print(f"\n📚 检索到的文档片段 (按RRF分数排序):")
            for i, (doc, score) in enumerate(zip(result.documents[:3], result.scores[:3]), 1):
                content = doc[:200] + "..." if len(doc) > 200 else doc
                print(f"\n📄 文档 {i} (RRF分数: {score:.3f}):")
                print(f"   {content}")
        
        # 显示调试信息
        if show_details and hasattr(result, '__dict__') and 'debug_info' in result.__dict__:
    ...
    def print_detailed_process(result: MultiQueryResult):
        ...
        if hasattr(result, '__dict__') and 'debug_info' in result.__dict__:
            ...
            if debug_info.get('query_results'):
                ...
                for i, query_result in enumerate(debug_info['query_results'], 1):
                    ...
                    if query_result['documents']:
                        print(f"   前3个文档:")
                        for j, (doc, score) in enumerate(zip(query_result['documents'][:3], query_result['scores'][:3])):
                            preview = doc[:100] + "..." if len(doc) > 100 else doc
            ...
            if debug_info.get('final_ranking'):
                ...
                print(f"   最终排序前5个文档:")
                for i, (doc_preview, rrf_score) in enumerate(debug_info['final_ranking'], 1):
    ...
    def print_comparison_result(result: ComparisonResult):
        ...
        print(f"\n📊 性能指标对比:")
        print(f"传统RAG - 文档数: {len(result.traditional.documents)}, 时间: {result.traditional.processing_time:.3f}秒")
        print(f"多查询RAG - 文档数: {len(result.multi_query.documents)}, 时间: {result.multi_query.processing_time:.3f}秒")
        ...
        try:
            ...
            if user_input.lower().startswith('debug '):
                ...
                if query:
                    ...
                    result = rag_system.query(query, show_process=True)
                    evaluator.print_query_result(result, "多查询RAG结果（调试模式）", show_details=True)
