...
def demo_pipeline_prompt_template():
    ...
    conclusion_template = PromptTemplate(
        input_variables=["deadline"],
        template="请在 {deadline} 之前完成，谢谢！"
    )
    ...
    try:
        ...
        data = {
            ...
            'data_points': [
                ...
                {'name': '客户满意度', 'value': 95, 'type': '质量', 'description': '客户反馈评分'},
...
def demo_template_management_best_practices():
    ...
    def validate_template_variables(template: PromptTemplate, required_vars: List[str]) -> bool:
        """验证模板变量"""
    ...
    if simple_template:
        is_valid = validate_template_variables(
            simple_template, 
            ['role', 'task_description', 'requirements']
        )
