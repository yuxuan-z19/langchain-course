...
class HumanInLoopClient:
    ...
    def send_chat_message(
        self, message: str, thread_id: str = "demo_session", demo_type: str = "basic"
    ) -> Dict[str, Any]:
        """发送聊天消息"""
        try:
            payload = {
    ...
    def resume_execution(self, thread_id: str, human_input: str) -> Dict[str, Any]:
        """恢复执行"""
        try:
            payload = {"thread_id": thread_id, "human_input": human_input}
    ...
    def check_status(self, thread_id: str) -> Dict[str, Any]:
        ...
        try:
            response = self.session.get(
                f"{self.base_url}/status/{thread_id}", timeout=10
            )
            response.raise_for_status()
            return response.json()
        ...
        while True:
            try:
                ...
                if user_input.lower() == "status":
                    ...
                    print(
                        f"📊 状态: {'🔴 中断' if status['is_interrupted'] else '🟢 正常'}"
                    )
                ...
                # 选择演示类型
                demo_type = (
                    input("演示类型 (basic/search/confirmation) [basic]: ").strip()
                    or "basic"
                )
        ...
        for thread_id in thread_ids:
            try:
                ...
                if status["conversation_state"]:
                    session_status = status["conversation_state"].get(
