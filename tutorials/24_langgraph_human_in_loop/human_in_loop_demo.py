...
class HumanInLoopChatbot:
        ...
        def chatbot(state: State):
            ...
            if has_recent_tool_result:
                ...
                search_result_found = any(
                    ...
                    and ("搜索结果" in msg.content or "🔍" in msg.content)
                ...
                if search_result_found:
                    ...
                    guidance_msg = HumanMessage(
                        content="请总结上述搜索结果，提取关键信息并给出简洁的回答。不要再次调用任何工具。"
                    )
    ...
    def resume_execution(
        self, human_input: str, thread_id: str = "default"
    ) -> List[Dict]:
        """恢复执行并提供人工输入
        ...
        try:
            ...
            events = []
            for event in self.graph.stream(
                Command(resume={"data": human_input}), config, stream_mode="values"
            ):
                events.append(event)
    ...
    while chatbot.check_interrupt_status("demo_thread_1"):
        ...
        try:
            ...
            if choice in ["1", "2", "3", "4"]:
                ...
                # 恢复执行
                chatbot.resume_execution(
                    f"用户选择: {selected_method}", "demo_thread_1"
                )
                break
            else:
