...
class ChatInput(BaseModel):
    ...
    demo_type: str = Field(default="basic", description="演示类型：basic, search, confirmation")
...
class HumanInLoopWebInterface:
    ...
    def _add_routes(self):
        ...
        async def chat_endpoint(chat_input: ChatInput, background_tasks: BackgroundTasks):
            """聊天端点"""
            try:
                # 初始化会话状态
    ...
    def _run_chat_with_interrupt_check(self, message: str, thread_id: str, demo_type: str) -> ChatOutput:
        """运行聊天并检查中断状态"""
        try:
            # 根据演示类型构造不同的提示
            if demo_type == "search":
