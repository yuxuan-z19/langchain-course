...
try:
    ...
    from utils.config import load_embedding_config, load_environment
...
class QueryRewriter:
    ...
    def rewrite_query(
        self, query: str, strategy: str = "standardize"
    ) -> QueryRewriteResult:
        """重写查询"""
...
class VectorDatabase:
    ...
    def __init__(
        self,
        collection_name: str = "query_rewrite_demo",
        persist_directory: str = "./demo_query_rewrite_db",
    ):
...
class QueryRewriteRAG:
    ...
    def __init__(
        self, llm_model, vector_db: VectorDatabase, query_rewriter: QueryRewriter
    ):
        """初始化查询重写RAG系统
    ...
    def query_with_rewrite(
        self, query: str, strategy: str = "standardize", top_k: int = 3
    ) -> Tuple[QueryRewriteResult, RAGResult]:
        """使用查询重写的RAG检索"""
        start_time = time.time()
...
class PerformanceEvaluator:
    ...
    def evaluate_strategies(self, rag_system: QueryRewriteRAG) -> Dict[str, Any]:
        ...
        for strategy in strategies:
            ...
            for query in self.test_queries:
                try:
                    rewrite_result, rag_result = rag_system.query_with_rewrite(
                        query, strategy
                    )
    ...
    def compare_with_traditional(self, rag_system: QueryRewriteRAG) -> Dict[str, Any]:
        ...
        for query in self.test_queries:
            try:
                ...
                # 查询重写RAG
                rewrite_result, rewrite_rag_result = rag_system.query_with_rewrite(
                    query, "standardize"
                )
    ...
    def print_evaluation_report(
        self, strategy_results: Dict[str, Any], comparison_results: Dict[str, Any]
    ):
        """打印评估报告"""
        print("\n" + "=" * 80)
        print("查询重写RAG系统评估报告")
        ...
        for strategy, results in strategy_results.items():
            if results:
                avg_confidence = sum(
                    r["rewrite_result"].confidence for r in results
                ) / len(results)
                avg_processing_time = sum(
                    r["rewrite_result"].processing_time for r in results
                ) / len(results)
        ...
        if comparison_results:
            traditional_times = [
                r["traditional"].processing_time for r in comparison_results
            ]
            rewrite_times = [
                r["rewrite"]["rag_result"].processing_time for r in comparison_results
            ]
            ...
            print(
                f"时间开销增加: {((avg_rewrite_time - avg_traditional_time) / avg_traditional_time * 100):.1f}%"
            )
            ...
            for i, result in enumerate(comparison_results[:3]):
                ...
                print(
                    f"重写RAG检索到 {len(result['rewrite']['rag_result'].documents)} 个文档"
                )
                ...
                try:
                    if strategy == "traditional":
                        ...
                        if rag_result.documents:
                            print("\n相关文档:")
                            for i, (doc, score) in enumerate(
                                zip(rag_result.documents, rag_result.scores)
                            ):
                                print(f"{i+1}. (相似度: {score:.3f}) {doc[:100]}...")
