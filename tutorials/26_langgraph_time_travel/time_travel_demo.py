...
class TimeTravelChatbot:
    ...
    def chat_with_time_travel(self, user_input: str, config: Dict[str, Any]) -> Dict[str, Any]:
        """与时间旅行聊天机器人对话"""
        try:
            # 创建用户消息
            ...
            for chunk in self.graph.stream(input_state, config, stream_mode="values"):
                final_state = chunk
                print(f"步骤 {chunk.get('step_count', 0)}: {chunk.get('current_task', '未知任务')}")
    ...
    def get_state_history(self, config: Dict[str, Any], limit: int = 10) -> List[CheckpointInfo]:
        """获取状态历史
        ...
        try:
            ...
            for i, state in enumerate(state_history):
                checkpoint_id = state.config.get("configurable", {}).get("checkpoint_id", f"checkpoint_{i}")
                thread_id = state.config.get("configurable", {}).get("thread_id", "unknown")
    ...
    def get_checkpoint_state(self, config: Dict[str, Any], checkpoint_id: Optional[str] = None) -> Optional[Dict[str, Any]]:
        """获取特定检查点的状态
    ...
    def update_state(self, config: Dict[str, Any], state_update: Dict[str, Any], as_node: Optional[str] = None) -> bool:
        """更新图状态
    ...
    def resume_from_checkpoint(self, config: Dict[str, Any], checkpoint_id: str, new_input: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """从特定检查点恢复执行
    ...
    def display_state_history(self, config: Dict[str, Any], limit: int = 5):
        ...
        for i, checkpoint in enumerate(history):
            ...
            if checkpoint.metadata:
                print(f"    元数据: {json.dumps(checkpoint.metadata, ensure_ascii=False, indent=6)}")
    
    def display_checkpoint_details(self, config: Dict[str, Any], checkpoint_id: Optional[str] = None):
        """显示检查点详细信息
...
def demonstrate_basic_time_travel():
    ...
    config = {
        "configurable": {
            "thread_id": "demo_basic_time_travel"
        }
    }
...
def demonstrate_checkpoint_operations():
    ...
    if len(history) >= 2:
        ...
        resume_result = chatbot.resume_from_checkpoint(
            ...
            {"messages": [HumanMessage(content="从这个时间点开始，请告诉我一个有趣的事实")]}
        )
...
def demonstrate_state_modification():
    ...
    config = {
        "configurable": {
            "thread_id": "demo_state_modification"
        }
    }
    ...
    if success:
        ...
        print("\n5. 基于修改后的状态继续对话...")
        result2 = chatbot.chat_with_time_travel("我应该从哪种编程语言开始学习？", config)
        print(f"助手回复: {result2['response']}")
    else:
        print("状态修改失败！")
...
def demonstrate_branch_creation():
    ...
    if len(main_history) >= 1:
        ...
        branch_config = {
            "configurable": {
                "thread_id": "demo_branch_alternative"
            }
        }
    ...
    while True:
        try:
            ...
            if user_input.lower() == 'quit':
                ...
                result = chatbot.resume_from_checkpoint(config, checkpoint_id, new_input)
                if result["success"]:
                    print("从检查点恢复执行成功！")
                else:
                ...
                if choice == '1':
                    ...
                    if key and value:
                        chatbot.update_state(config, {
                            "metadata": {key: value, "modified_at": datetime.now().isoformat()}
                        })
            else:
                # 正常对话
                result = chatbot.chat_with_time_travel(user_input, config)
