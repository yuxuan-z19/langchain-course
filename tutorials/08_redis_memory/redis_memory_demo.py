...
class RedisMemoryStore:
    ...
    def get_memory_stats(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # 获取内存使用
            memory_usage = (
                self.redis_client.memory_usage(key) if message_count > 0 else 0
            )
...
class RedisChatMemory:
    ...
    def __init__(
        self, redis_store: RedisMemoryStore, user_id: str, max_messages: int = 20
    ):
...
def demo_redis_connection():
    ...
    try:
        # 创建Redis存储
        redis_store = RedisMemoryStore(host="localhost", port=6379, db=0)
...
请回答:"""
    ...
    # 第二轮对话
    conversations_2 = ["你还记得我的名字吗？", "我们之前聊到了什么？"]
...
def demo_migration_from_inmemory():
    ...
    # 比较消息内容
    print("\n消息对比:")
    for i, (orig, migrated) in enumerate(
        zip(inmemory_history.messages, redis_messages)
    ):
