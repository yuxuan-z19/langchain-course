...
class DashScopeEmbeddings(Embeddings):
    ...
    def _call_api(self, texts: List[str]) -> List[List[float]]:
        ...
        try:
            ...
            logger.debug(
                f"DashScope API调用成功，返回结果: {len(result.get('output', {}).get('embeddings', []))} 个嵌入向量"
            )
...
class OpenAIEmbeddings(Embeddings):
    ...
    def _call_api(self, texts: List[str]) -> List[List[float]]:
        ...
        try:
            ...
            result = response.json()
            logger.debug(
                f"OpenAI API调用成功，返回结果: {len(result.get('data', []))} 个嵌入向量"
            )
...
class ParentChildRAG:
    ...
    def _setup_embedding_model(self) -> Embeddings:
        ...
        try:
            ...
            if not config.get("api_key"):
                raise ValueError(
                    "未找到有效的API密钥，请检查.env文件中的EMBEDDING_API_KEY"
                )
            ...
            logger.info(
                f"配置信息 - API密钥: {'已设置' if api_key else '未设置'}, 模型: {model_name}, 基础URL: {base_url}"
            )
    ...
    def add_documents(self, chunks: List[Dict[str, Any]]) -> None:
        ...
        try:
            self.collection.add(
                ids=child_ids, documents=child_contents, metadatas=metadatas
            )
...
class TraditionalRAG:
    ...
    def add_document(self, text: str, source: str = "") -> None:
        ...
        metadatas = [
            {"source": source, "chunk_index": i, "chunk_length": len(chunk)}
...
class ParentChildDemo:
    ...
    def run_chunking_strategy_comparison(self, text: str) -> Dict[str, Any]:
        ...
        for strategy_name, config in self.chunker_configs.items():
            ...
            avg_parent_length = sum(
                chunk["metadata"]["parent_length"] for chunk in chunks
            ) / len(chunks)
            avg_child_length = sum(
                chunk["metadata"]["child_length"] for chunk in chunks
            ) / len(chunks)
    ...
    def run_retrieval_comparison(self, text: str) -> Dict[str, Any]:
        ...
        for query in self.test_queries:
            ...
            # 计算统计信息
            pc_avg_score = (
                sum(r["similarity_score"] for r in pc_results) / len(pc_results)
                if pc_results
                else 0
            )
            trad_avg_score = (
                sum(r["similarity_score"] for r in trad_results) / len(trad_results)
                if trad_results
                else 0
            )
            ...
            pc_avg_length = (
                sum(
                    len(r.get("parent_content", r.get("child_content", "")))
                    for r in pc_results
                )
                / len(pc_results)
                if pc_results
            ...
            trad_avg_length = (
                sum(len(r["content"]) for r in trad_results) / len(trad_results)
                if trad_results
                else 0
            )
    ...
    def print_chunking_comparison(self, results: Dict[str, Any]) -> None:
        ...
        for strategy_name, result in results.items():
            ...
            print(f"\n【{strategy_name}策略】")
            print(
                f"  父文档配置: {config.parent_chunk_size}字符, 重叠{config.parent_overlap}字符"
            )
            print(
                f"  子文档配置: {config.child_chunk_size}字符, 重叠{config.child_overlap}字符"
            )
    ...
    def print_retrieval_comparison(self, results: Dict[str, Any]) -> None:
        ...
        print(
            f"  父子分段RAG平均内容长度: {summary['parent_child_avg_length']:.0f}字符"
        )
        print(f"  传统RAG平均内容长度: {summary['traditional_avg_length']:.0f}字符")

        improvement = (
            (
                (summary["parent_child_avg_score"] - summary["traditional_avg_score"])
                / summary["traditional_avg_score"]
                * 100
            )
            if summary["traditional_avg_score"] > 0
            else 0
        )
        ...
        for i, query_result in enumerate(results["queries"][:3]):
            ...
            if query_result["parent_child_results"]:
                ...
                print(
                    f"  父子分段最佳匹配: {best_pc.get('child_content', '')[:150]}..."
                )
