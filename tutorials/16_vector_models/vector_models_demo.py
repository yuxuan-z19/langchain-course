...
class VectorModelsDemo:
    ...
    def setup_embedding_models(self) -> Dict[str, Any]:
        ...
        try:
            ...
            print(f"📋 向量模型配置加载成功")
            print(
                f"   🔑 API密钥: {'已配置' if embedding_config.get('api_key') else '未配置'}"
            )
    ...
    def load_civil_code_pdf(self):
        ...
        try:
            ...
            # 过滤掉过短的文档片段
            filtered_docs = [
                doc for doc in split_docs if len(doc.page_content.strip()) > 50
            ]
    ...
    def create_embedding_visualization(self, all_embeddings: Dict[str, Dict]):
        ...
        try:
            ...
            if model_names:
                dimensions = [
                    all_embeddings[name]["embeddings"].shape[1] for name in model_names
                ]
                axes[0, 0].bar(
                    model_names, dimensions, color=["#FF6B6B", "#4ECDC4", "#45B7D1"]
                )
            ...
            if model_names:
                ...
                im = axes[1, 0].imshow(
                    similarity_matrix, cmap="coolwarm", vmin=-1, vmax=1
                )
            ...
            if model_names:
                ...
                axes[1, 1].plot(
                    mean_values[:50], label="均值", alpha=0.7
                )  # 只显示前50个维度
    ...
    def demo_civil_code_vectorization(self):
        ...
        legal_categories = {
            "总则条文": [
                text for text in self.test_texts if "总则" in text or "第一编" in text
            ],
            "物权条文": [
                text for text in self.test_texts if "物权" in text or "第二编" in text
            ],
            "合同条文": [
                text for text in self.test_texts if "合同" in text or "第三编" in text
            ],
            "人格权条文": [
                text for text in self.test_texts if "人格权" in text or "第四编" in text
            ],
            "婚姻家庭条文": [
                text
                for text in self.test_texts
                if "婚姻" in text or "家庭" in text or "第五编" in text
            ],
            "继承条文": [
                text for text in self.test_texts if "继承" in text or "第六编" in text
            ],
            "侵权责任条文": [
                text
                for text in self.test_texts
                if "侵权" in text or "责任" in text or "第七编" in text
            ],
        }
            ...
            if len(embeddings) >= 2:
                ...
                for i in range(len(similarity_matrix)):
                    for j in range(len(similarity_matrix[i])):
                        if i != j:
                            print(
                                f"      片段{i+1} vs 片段{j+1}: {similarity_matrix[i][j]:.4f}"
                            )
            print()
    ...
    def print_summary_report(self, performance_results: Dict[str, Dict]):
        ...
        if performance_results:
            ...
            sorted_models = sorted(
                performance_results.items(), key=lambda x: x[1]["avg_time"]
            )
            ...
            for i, (model_name, stats) in enumerate(sorted_models, 1):
                ...
                print(
                    f"     ✅ 成功率: {stats['successful_count']}/{len(self.test_texts)}"
                )
