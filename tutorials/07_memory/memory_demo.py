...
def demo_basic_memory_operations():
    ...
    history.add_ai_message(
        "Python是一门很棒的编程语言！它简单易学，功能强大。你想从哪个方面开始学习呢？"
    )
...
def demo_memory_with_llm():
    ...
    # 添加系统消息
    system_msg = (
        "你是一个有用的AI助手。请记住用户告诉你的信息，并在后续对话中使用这些信息。"
    )
...
def demo_memory_capacity_limits():
    ...
    for i in range(15):
        ...
        if len(history.messages) > max_messages:
            ...
            ][-max_messages + len(system_messages) :]
...
def demo_interactive_chat():
    ...
    print(
        f"总对话轮数: {len([msg for msg in history.messages if isinstance(msg, HumanMessage)])}"
    )
    print(f"总消息数: {len(history.messages)}")
...
def demo_memory_persistence():
    ...
    print(f"  会话B消息数: {len(session_b.messages)}")
    print(
        f"  会话A不包含会话B的内容: {all(msg.content != '我是程序员' for msg in session_a.messages)}"
    )
