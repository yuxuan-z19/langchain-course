...
class CompressionStrategy(ABC):
    ...
    @abstractmethod
    def compress(
        self, messages: List[BaseMessage], config: CompressionConfig
    ) -> List[BaseMessage]:
        """执行压缩"""
        pass
...
""",
        ...
        try:
            ...
            summary_response = chain.invoke(
                {"conversation": conversation_text, "max_length": config.summary_length}
            )
...
class ProgressiveCompressor(CompressionStrategy):
    ...
    def compress(
        ...
        middle = messages[
            -config.preserve_recent * 2 : -config.preserve_recent
        ]  # 中期的，轻度压缩
...
class MemoryCompressor:
    ...
    def compress(self, strategy: str = "progressive") -> List[BaseMessage]:
        ...
        compression_record = {
            ...
            "compression_ratio": (
                len(compressed_messages) / original_count if original_count > 0 else 0
            ),
        }
        ...
        print(
            f"📊 Token减少: {original_tokens} -> {compression_record['compressed_tokens']}"
        )
    ...
    def get_memory_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_compression": (
                self.compression_history[-1] if self.compression_history else None
            ),
        }
        ...
        if self.storage_backend == "redis":
            try:
                ...
                self.redis_client.set(
                    "compression_history", json.dumps(self.compression_history)
                )
            except Exception as e:
...
def demo_basic_compression():
    ...
    config = CompressionConfig(
        max_messages=10, max_tokens=1000, compression_ratio=0.5, preserve_recent=3
    )
    ...
    conversations = [
        ...
        (
            "assistant",
            "你好！我很乐意帮助你学习Python。Python是一门非常适合初学者的编程语言。",
        ),
        ("user", "我应该从哪里开始？"),
    ...
    if compressor.compression_history:
        ...
        for record in compressor.compression_history:
            print(
                f"   {record['timestamp'][:19]} | {record['strategy']} | "
...
def demo_compression_strategies():
    ...
    test_messages = [
        ...
        AIMessage(
            "你好！机器学习是人工智能的一个重要分支，它让计算机能够从数据中学习。"
        ),
        HumanMessage("有哪些主要的机器学习算法？"),
        AIMessage(
            "主要包括监督学习（如线性回归、决策树）、无监督学习（如聚类、降维）和强化学习。"
        ),
        HumanMessage("今天天气很好"),
    ...
    for strategy in strategies:
        ...
        for i, msg in enumerate(compressed):
            ...
            compressed_flag = (
                "[压缩]" if msg.additional_kwargs.get("compressed") else ""
            )
...
def demo_compression_evaluation():
    ...
    mixed_conversations = [
        ...
        (
            "assistant",
            "当然！你可以使用memory_profiler库来监控内存使用，或者使用gc模块检查垃圾回收。",
        ),
        ("user", "这个解决方案很有用，谢谢！"),
        ("assistant", "不客气！如果还有其他技术问题，随时可以问我。"),
    ]

    config = CompressionConfig(max_messages=8, preserve_recent=3, min_importance=0.6)
    ...
    print("\n📊 压缩策略评估结果:")
    print(
        f"{'策略':<12} {'压缩比':<8} {'Token减少':<10} {'最终消息数':<10} {'最终Token数':<10}"
    )
    print("-" * 60)

    for strategy, results in evaluation_results.items():
        print(
            f"{strategy:<12} {results['compression_ratio']:<8.2f} "
    ...
    # 推荐最佳策略
    best_strategy = min(
        evaluation_results.keys(), key=lambda x: evaluation_results[x]["final_tokens"]
    )
...
def demo_real_world_scenario():
    ...
    customer_service_chat = [
        ...
        (
            "assistant",
            "1. 登录域名管理后台 2. 找到DNS设置 3. 修改A记录指向你的服务器IP",
        ),
        ("user", "我不知道服务器IP"),
    ...
    # 配置压缩器
    config = CompressionConfig(max_messages=12, preserve_recent=4, min_importance=0.7)
    ...
    if compressor.compression_history:
        ...
        print(
            f"   最后压缩: {last_compression['original_count']} -> {last_compression['compressed_count']} 条"
        )
