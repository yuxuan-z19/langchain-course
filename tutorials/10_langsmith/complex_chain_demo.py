...
# 导入项目统一配置管理
from utils.config import load_environment, load_deepseek_config, setup_environment_variables
...
class LangSmithComplexConfig:
    ...
    def __init__(self):
        ...
        # 验证必要的配置
        if not self.api_key:
            raise ValueError("LANGCHAIN_API_KEY 环境变量未设置，请在项目根目录的 .env 文件中配置")
        if not (self.api_key.startswith('ls__') or self.api_key.startswith('lsv2_')):
            raise ValueError("LANGCHAIN_API_KEY 格式不正确，应该以 'ls__' 或 'lsv2_' 开头")
...
class ComplexChainProcessor:
    ...
    def _process_multi_step(self, query: str) -> str:
        ...
        analysis_prompt = ChatPromptTemplate.from_messages([
            ("system", "分析用户查询，识别需要执行的步骤。将复杂任务分解为具体的执行步骤。"),
            ("human", "查询: {query}\n\n请分析这个查询需要哪些步骤来完成：")
        ])
        ...
        execution_prompt = ChatPromptTemplate.from_messages([
            ("system", "你是一个任务执行助手。根据分析结果，逐步执行任务并提供详细的执行过程。"),
            ("human", "原始查询: {query}\n\n步骤分析: {steps}\n\n请按步骤执行任务：")
        ])
        ...
        final_result = validation_chain.invoke({
            "query": query,
            "result": result
        })
    ...
    def _process_calculation(self, query: str) -> str:
        ...
        extraction_prompt = ChatPromptTemplate.from_messages([
            ("system", "从用户的查询中提取数学表达式。只返回可以直接计算的表达式，如：2+3*4"),
            ("human", "{query}")
        ])
    ...
    def _process_creative(self, query: str) -> str:
        ...
        creative_prompt = ChatPromptTemplate.from_messages([
            ("system", "你是一个创意写作助手。请根据用户的要求进行创意写作，保持内容积极向上。"),
            ("human", "{query}")
        ])
