...
def demo_basic_dict_schema():
    ...
    try:
        ...
        dict_schema = {
            ...
            "type": "object",
            "properties": {
                "answer": {"type": "string", "description": "用户问题的答案"},
                "confidence": {
                    "type": "number",
                ...
                "category": {"type": "string", "description": "问题的分类"},
            },
...
def demo_tool_calling():
    ...
    try:
        ...
        ai_msg = model_with_tools.invoke(
            [
                SystemMessage(
                    content="请从给定文本中提取人物信息，使用PersonInfo工具。"
                ),
...
请确保返回有效的JSON格式。"""
        ...
        try:
            ...
            # 验证JSON结构
            if (
                "random_numbers" in parsed_result
                and "sum" in parsed_result
                and "average" in parsed_result
            ):
                print("\n✓ JSON结构验证成功")
...
请确保返回有效的JSON格式。"""
            ...
            try:
                ...
                print(f"原始结果: {result}")
                print(
                    f"清理后结果: {cleaned_result if 'cleaned_result' in locals() else 'N/A'}"
                )
                print("\n💡 建议: 调整提示词或重新尝试")
    ...
    try:
        ...
        for i, test_input in enumerate(test_cases, 1):
            ...
            try:
                ...
                print(f"成功: {result}")
                print(
                    f"验证通过: number={result.number}, percentage={result.percentage}, category={result.category}"
                )
            except Exception as e:
                print(f"错误: {e}")
